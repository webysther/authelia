...
[token_endpoint_auth_method](#tokenendpointauthmethod) that uses a credential type that isn't a secret (i.e. a key), and
...
where the configured [grant_types](#granttypes) includes the `client_credentials` grant in which case arbitrary scopes are
expected,
