...
frontend fe_http
    ...
    http-request lua.auth-intercept be_authelia /api/authz/forward-auth HEAD * remote_user,remote-user,remote-groups,remote-name,remote-email - if protected-frontends
...
frontend fe_http
    ...
    # Protect endpoints with haproxy-auth-request and Authelia
    http-request lua.auth-intercept be_authelia_proxy /api/authz/forward-auth HEAD * remote_user,remote-user,remote-groups,remote-name,remote-email - if protected-frontends
    http-request redirect location %[var(txn.auth_response_location)] if protected-frontends !{ var(txn.auth_response_successful) -m bool }
