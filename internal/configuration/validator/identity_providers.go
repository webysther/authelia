...
func validateOIDCClientScopesSpecialBearerAuthz(c int, config *schema.IdentityProvidersOpenIDConnect, validator *schema.StructValidator) bool {
	...
	} else if !utils.IsStringSliceContainsAll(config.Clients[c].GrantTypes, validOIDCClientGrantTypesBearerAuthz) ||
		!utils.IsStringSliceContainsAny(config.Clients[c].GrantTypes, validOIDCClientGrantTypesBearerAuthz) {
		validator.Push(fmt.Errorf(errFmtOIDCClientInvalidEntriesScope, config.Clients[c].ID, attrOIDCGrantTypes, strJoinAnd(validOIDCClientGrantTypesBearerAuthz), oidc.ScopeAutheliaBearerAuthz, strJoinAnd(config.Clients[c].GrantTypes)))
	}
	...
	if config.Clients[c].Public {
		...
		case "", oidc.ClientAuthMethodNone:
			validator.Push(fmt.Errorf(errFmtOIDCClientOptionMustScopeClientType, config.Clients[c].ID, attrOIDCTokenAuthMethod, strJoinOr([]string{oidc.ClientAuthMethodClientSecretBasic, oidc.ClientAuthMethodClientSecretPost, oidc.ClientAuthMethodClientSecretJWT, oidc.ClientAuthMethodPrivateKeyJWT}), oidc.ScopeAutheliaBearerAuthz, "confidential", config.Clients[c].TokenEndpointAuthMethod))
		}
	}
